...
\newcommand{\EndMacros}{
    \node[white, anchor=north, yshift=-0.5em, font=\bfseries] at (botC) {Magnetização $\symbf{\phi}$};
    \node[white, anchor=north, yshift=-0.5em, font=\bfseries] at (\magM,-\Ly) {$\symbf{-1}$};
    \node[white, anchor=north, yshift=-0.5em, font=\bfseries] at (\magP,-\Ly) {$\symbf{+1}$};
    \node[white, anchor=south, yshift=0.5em, font=\bfseries] at (0,-\Ly) {$\symbf{0}$};

    \draw[gray, very thick] (\magM,-\Ly) -- (\magM,-\Ly+0.5);
    \draw[gray, very thick] (0,-\Ly) -- (0,-\Ly+0.5);
    \draw[gray, very thick] (\magP,-\Ly) -- (\magP,-\Ly+0.5);
    \node[white, very thick, anchor=south east] at (\magM,0) {$\symbf{V=0}$};
    \draw[white, thick] (botL) rectangle (topR);
}

\newcommand{\Legend}{
    \begin{scope}[shift={(\magP,0)}]
        \draw[noise arrow, yshift=1em] (-1.5,3) to[bend left=10] (1.5,3);
        \node[white, anchor=north, yshift=1em] at (0,3) {Ruído};
        \draw[drift arrow, yshift=1em] (-2,0) -- (2,0)
            node[white, midway, below, anchor=north] {Drift};
        \draw[jump arrow, yshift=1em] (-2,-3) -- (2,-3)
            node[white, midway, below, anchor=north] {Efetivo};
    \end{scope}
}
...
\begin{tikzpicture}[x=1em, y=1em]
    ...
    \node[white,font=\bfseries] (ferro) at (0,-0.5*\Ly) {Mínimos Ferromagnéticos};
    \node[white,font=\bfseries] (para) at (0,0.5*\Ly) {Mínimo Paramagnético};
